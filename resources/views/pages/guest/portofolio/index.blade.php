...
@section('content')
    ...
    }" x-init="$watch('showProjectModal', value => value ? document.body.classList.add('overflow-hidden') : document.body.classList.remove('overflow-hidden'));
    $watch('showCertificateModal', value => value ? document.body.classList.add('overflow-hidden') : document.body.classList.remove('overflow-hidden'));"
                        ...
                        <div class="flex-1 w-full pt-2 md:pt-0">
                            <div class="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
                                ...
                                <div class="flex gap-3 relative z-10">
                                    {{-- Tombol Download CV --}}
                                    <a href="{{ route('portfolio.download', ['userId' => $user->id]) }}"
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-6">
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            ...
                            <button
                                onclick="window.open('https://twitter.com/intent/tweet?text=Check this portfolio&url={{ urlencode(request()->url()) }}', '_blank')"
                ...
                <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="flex items-center gap-3 mb-6">
                            <div class="p-2 bg-purple-500/10 border border-purple-500/20 rounded-lg">
                                <svg class="w-6 h-6 text-purple-400" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M4 6a2 2 0 012-2h2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 01-2 2h-2a2 2 0 01-2-2zM4 16a2 2 0 012-2h2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 01-2 2h-2a2 2 0 01-2-2z">
                                    </path>
                                </svg>
                            </div>
                        ...
                        @else
                            <div class="grid md:grid-cols-2 gap-6">
                                @foreach ($user->projects as $project)
                                    <div class="bg-slate-900 cursor-pointer border border-slate-800 rounded-xl overflow-hidden hover:border-slate-700 transition group"
                                        @click="selectedProject = {{ $project }}; showProjectModal = true">
                    ...
                    <div>
                        ...
                        @if ($user->certificates->isEmpty())
                            <div
                                class="bg-slate-900/60 cursor-pointer backdrop-blur-xl border border-slate-800 rounded-2xl p-12 text-center">
                                <div
                                    class="w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-600">
                                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="grid md:grid-cols-2 gap-4">
                                @foreach ($user->certificates as $cert)
                                    <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center gap-4 hover:border-slate-700 transition relative group cursor-pointer"
                                        @click="selectedCertificate = {{ $cert }}; showCertificateModal = true">
                                        <div
        ...
        <div x-show="showProjectModal" style="display: none;"
            ...
            <div
                ...
                <div class="relative h-64 bg-slate-800 flex-shrink-0">
                    <img :src="selectedProject?.image_path" class="w-full h-full object-cover">
                    <div class="absolute inset-0 bg-gradient-to-t from-slate-900 to-transparent"></div>
                    <button @click="showProjectModal = false"
                        class="absolute top-4 right-4 bg-black/50 text-white flex items-center justify-center p-2 rounded-full hover:bg-black/70 transition"><i
                            class="fa-solid fa-xmark text-[12px]"></i></button>
                </div>
                ...
                <div class="p-8 overflow-y-auto">
                    ...
                    {{-- External Link --}}
                    <template x-if="selectedProject?.project_link">
                        <a :href="selectedProject?.project_link" target="_blank"
                            ...
                            <i class="fa-solid fa-external-link-alt"></i> Visit Project
                        </a>
                    </template>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <template x-for="tag in selectedProject?.tags">
                            <span
                                class="px-3 py-1 bg-slate-800 text-slate-300 text-sm rounded-full border border-slate-700"
                                x-text="tag"></span>
                        </template>
                    </div>
                </div>
        ...
        <div x-show="showCertificateModal" style="display: none;"
            ...
            <div
                class="relative bg-slate-900 border border-slate-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
                <div class="p-8 text-center">
                    ...
                    <p class="text-emerald-400 text-sm mb-4 font-semibold"
                        x-text="selectedCertificate?.issuer_organization"></p>

                    <div class="inline-block px-4 py-2 rounded-lg bg-slate-800 border border-slate-700">
                        <p class="text-slate-400 text-xs">Issued on</p>
                        <p class="text-white font-medium text-sm" x-text="selectedCertificate?.date_issued"></p>
                    </div>

                    <div class="mt-8">
                        <button @click="showCertificateModal = false"
                            class="px-6 py-2 bg-slate-800 text-white rounded-lg text-sm font-bold hover:bg-slate-700 transition border border-slate-700">Close</button>
                    </div>
                </div>
            </div>
        </div>
